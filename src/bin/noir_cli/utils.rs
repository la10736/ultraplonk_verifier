...
//     out_file(output_vk.as_ref())?
//         .write_all(hex::encode(&vk).as_bytes())
//         .map_err(|_| CliError::CliError("Failed to write output file".to_string()))?;

//     let proof = std::fs::read(input_proof).map_err(|e| {
//         CliError::CliError(format!(
//             "Failed to read file: {:?}. Reason :{:?}",
//             input_proof, e
//         ))
...
//         .write_all(hex::encode(&proof).as_bytes())
//         .map_err(|_| CliError::CliError("Failed to write output file".to_string()))?;

//     let pubs: Vec<u8> = std::fs::read(input_pubs).map_err(|e| {
//         CliError::CliError(format!(
//             "Failed to read file: {:?}. Reason :{:?}",
//             input_pubs, e
//         ))
...
//     let pubs_vec = crate::verifier::convert_to_pub_inputs(&pubs)?;

//     let mut w = out_file(output_pubs.as_ref())?;
//     for p in pubs_vec {
//         w.write_all(hex::encode(&p).as_bytes())
//             .map_err(|_| CliError::CliError("Failed to write output file".to_string()))?;
//         w.write(b",")
//             .map_err(|_| CliError::CliError("Failed to write output file".to_string()))?;
//     }
